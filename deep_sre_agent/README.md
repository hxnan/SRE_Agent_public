...
### Wiki 子Agent（DeepWiki MCP）

使用 DeepWiki MCP 回答项目架构与代码问题：

1. 配置环境变量（默认使用 `sse`）：
   - `DEEPWIKI_MCP_MODE=http` 或 `sse`
   - `DEEPWIKI_MCP_URL=https://mcp.deepwiki.com/mcp`
   - `DEEPWIKI_REPO_URL=你的仓库URL`
2. 启动服务：在 `deep_sre_agent` 目录执行 `langgraph dev`。
3. 通过 Studio 或 Deepagents UI 提问：
   - 例如：“项目架构如何分层？核心入口文件在哪里？”
4. 子Agent 工具：
   - `wiki_read_structure`、`wiki_read_contents`、`wiki_ask_question`
5. 集成位置：子Agent 定义与注册见 `deep_sre_agent\agent.py`。
6. 客户端实现：已采用官方 Python SDK（`mcp`）连接，支持 `SSE` 与 `Streamable HTTP`，默认挂载点为 `/sse` 与 `/mcp`。如遇 HTTP 不兼容，可将 `DEEPWIKI_MCP_MODE` 设为 `sse`。
   - SDK 项目地址：https://github.com/modelcontextprotocol/python-sdk

### Prometheus 子Agent（prometheus-mcp）

使用 Prometheus MCP 进行指标/告警分析与 PromQL 查询：

1. 配置环境变量（默认使用 `http` Streamable-HTTP）：  
   - `PROM_MCP_MODE=http` 或 `sse`  
   - `PROM_MCP_URL=http://localhost:7081/mcp`  
   - `PROM_MCP_SSE_URL=http://localhost:7081/sse`（如需兼容SSE）  
   - `PROM_MCP_HEADERS={"Authorization":"Bearer xxx"}`（可选）  
   - 工具名可覆盖：`PROM_MCP_TOOL_QUERY`（默认 `prometheus_query`）、`PROM_MCP_TOOL_RULES`、`PROM_MCP_TOOL_ALERTS`、`PROM_MCP_TOOL_TARGETS`、`PROM_MCP_TOOL_SERIES`、`PROM_MCP_TOOL_METADATA`
2. 启动服务：在 `deep_sre_agent` 目录执行 `langgraph dev`。
3. 通过 Studio 或 Deepagents UI 提问：  
   - 例如：“过去10分钟 5xx 是否异常？”、“当前告警风暴涉及哪些实例？”  
4. 子Agent 工具：  
   - `prom_query_range`、`prom_query_instant`、`prom_list_rules`、`prom_list_alerts`、`prom_list_targets`、`prom_list_series`、`prom_metadata`
5. 集成位置：子Agent 定义与注册见 `deep_sre_agent\\agent.py`。
6. 客户端实现：使用官方 Python SDK（`mcp`）的 `http_client` 连接（Streamable-HTTP），可选支持 `SSE` 并在失败时自动回退至 HTTP。默认挂载点约定 `/mcp` 与 `/sse`。  
   - SDK 项目地址：https://github.com/modelcontextprotocol/python-sdk
